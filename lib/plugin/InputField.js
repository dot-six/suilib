...
const SPECIAL_KEYS_FUNCTION = {
    'Backspace': (v) => v.substring(0, v.length-1),
    'Space': (v) => v + ' '
};
...
class InputField extends Node {
    ...
    _register_events() {
        ...
        this.ev.on('key-up', (e) => {
            ...
            let lastLength = this.value.length;
            if (ch.length > 1) {
            ...
            if (this.value.length != lastLength) {
                this._cursorPosition += (this.value.length - lastLength);
            }
        });
    }
    ...
    render() {
        ...
        let charWidth = (this._elements.text.size.x / this._elements.text.content.length);
        if (!isFinite(charWidth)) charWidth = 0;
        let valueCursorPositionOffset = (charWidth * this._cursorPosition);
        this._elements.focusCursor.setPosition(this._elements.text.position.add(
            valueCursorPositionOffset, 0
        ));
